...
const ATTENDANCE_STATUS_SET = new Set<AttendanceStatus>(
  ATTENDANCE_STATUS_OPTIONS,
);

function hasSelectedAttendanceStatus(
  status: AttendanceItem["status"],
): status is AttendanceStatus {
  return (
    typeof status === "string" &&
    ATTENDANCE_STATUS_SET.has(status as AttendanceStatus)
  );
}
...
export default function BabyLionsAttendancePage() {
  ...
  const handleSave = async () => {
    ...
    const users = rows.flatMap((item) =>
      hasSelectedAttendanceStatus(item.status)
        ? [{ loginId: item.loginId, status: item.status }]
        : [],
    );

    if (users.length === 0) {
      setError("저장할 출석 상태를 먼저 선택해 주세요.");
      setMessage("");
      return;
    }
    ...
    >
      <div className="space-y-6">
        <section className="rounded-xl border border-[#43485a] bg-[#363c4a] p-4">
          <div className="grid gap-3 md:grid-cols-[1fr_1fr_auto]">
            ...
            <button
              type="button"
              onClick={() => void load()}
              className="h-11 rounded-lg bg-main-1 px-4 text-sm cursor-pointer font-semibold"
            >
              출석 조회
            </button>
          </div>
        ...
        <section className="rounded-xl border border-[#43485a] bg-[#363c4a] p-4">
          ...
          {isLoading && (
            <p className="mt-3 text-sm text-gray-4">불러오는 중...</p>
          )}
          ...
          {rows.length > 0 && (
            <div className="mt-3 space-y-3">
                ...
                <table className="min-w-full text-left text-sm">
                  ...
                  <tbody>
                    {rows.map((item) => (
                      <tr
                        key={item.userId}
                        className="border-b border-[#41485a] text-gray-2"
                      >
                        <td className="px-3 py-2">{item.studentNo}</td>
                        <td className="px-3 py-2">{item.name}</td>
                        {ATTENDANCE_STATUS_OPTIONS.map((status) => {
                          const active = item.status === status;
                          const activeClassByStatus: Record<
                            AttendanceStatus,
                            string
                          > = {
                          ...
                          return (
                            <td key={status} className="px-3 py-2 text-center">
                              <button
                                type="button"
                                onClick={() =>
                                  handleChangeStatus(item.userId, status)
                                }
                                aria-pressed={active}
              ...
              <p className="text-xs text-gray-4">
                상태를 선택한 인원만 저장되며, 선택하지 않은 인원은 요청에
                포함되지 않습니다.
              </p>
            </div>
          )}
        ...
        <div className="flex justify-end">
          <button
            ...
            className="rounded-lg bg-main-1 px-5 py-2 text-sm font-semibold cursor-pointer disabled:opacity-60"
          >
            {isSaving ? "저장 중..." : "출석 저장"}
          </button>
        </div>
